...
void URLImageDownloadToFileWithURLDownloadToFile(std::string url, int identity) {
	std::string filename = std::to_string(identity) + ".png";
	LPCWSTR URL = StringToLPCWSTR(url);
	LPCWSTR FILENAME = StringToLPCWSTR(filename);
...
int main() {
	...
	char inputInformation[1000];
	memset(inputInformation, 0, sizeof inputInformation);
	inputbox_getline("Information! Choose range",
	...
	int size = strlen(inputInformation), isCatchedSpace = 0;
	...
	memset(inputInformation, 0, sizeof inputInformation);
	inputbox_getline("Information! Choose downloader",
	...
	memset(inputInformation, 0, sizeof inputInformation);
	inputbox_getline("Information! Choose model",
	...
	for (int i = beginID; i < beginID + imagesNumber; ++i) {
		...
		case 4:
			...
			break;
		case 5:
			URLImageDownloadToFileWithURLDownloadToFile(URL_TABLE[rand() % URLTABLE_SIZE], i);
		default: break;
		}
